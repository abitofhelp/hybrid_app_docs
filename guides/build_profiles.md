...
Hybrid App Ada supports multiple build profiles for different target platforms. Each profile configures:
...
```bash
gprbuild -P hybrid_app_ada.gpr -XHYBRID_APP_PROFILE=embedded
```
...
3. **Config Package**: Each profile has its own `Hybrid_App_Ada_Config` package in `config/profiles/<profile>/hybrid_app_ada_config.ads`
...
```ada
package Hybrid_App_Ada_Config is
   pragma Pure;
...
end Hybrid_App_Ada_Config;
```

## Adding a New Profile

1. Create directory: `config/profiles/<profile_name>/`
2. Create config file: `hybrid_app_ada_config.ads` with appropriate values
3. Add the profile to the `Profile_Type` in `hybrid_app_ada.gpr`
